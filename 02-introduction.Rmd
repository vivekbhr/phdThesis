...
exception to the crick's definition of central dogma has been observed
...
A *gene* is a major component of the central dogma. The overall definition
...
transfers its encoded information into the protein product, through
*transcription*, and *translation* (Appendix B1). DNA elements that
...
RNAs (e.g. antisense RNA ^5^, micro-RNA ^6^, long non-coding RNA
...
through the cell cycle, or across multiple generations. Such mechanisms are
collectively studied under the term "epigenetics" ^9^.

In order to understand the regulation of gene expression, it is important
...
following sections, I will discuss the various scales of gene
...
^15,16^. Genome-wide analyses have also proposed "biochemical
...
transposons, have been classically known to be involved in the regulation of 
protein-coding genes ^18^ (Appendix B2, Table-1).
...
DNA elements, DNA modifications (such as methylation), transcription 
factors (TFs), and histone marks. For example, during embryonic
development, pluripotency genes Oct3/4 are switched off in a multi-step
process where first a repressor binds to its promoter, turning off
...
looping is regulated via insulators, adding another layer to gene
regulation.
...
enhancer-promoter interactions in the early 1990s ^23^. Since then,
various techniques have been developed to study long-range interactions
via chromatin looping (see section on the analyses of chromosome
...
compartments ^24^ and chromatin domains, referred as **topologically associating 
domains (TADs)** ^25^. Studies performed later revealed that 
chromatin is hierarchically organized, where the domains with similar
chromatin signatures are spatially clustered. At the highest level of this 
clustering, the genome can be divided into A and B **compartments**, which
...
complexes (cohesin and condensin) has received the most evidence, and this
...
*in-silico* evidence, the *in-vivo* evidence linking extrusion to loop
...
surprisingly, they also showed that compartments as well as histone
marks remain unaffected upon loop depletion, suggesting an independent
mechanism behind the segregation of compartments. Another interesting point
...
have the motor activity ^43^ and recently, its *in-vivo* loop extrusion
...
The process of mammalian X-chromosome inactivation (XCI) serves as an excellent
...
X-chromosomes is randomly inactivated during differentiation ^49^. In
...
coincides with the downregulation of pluripotency factors such as
...
regulated by its antisense RNA Tsix, and positively regulated through a
...
Tsix has been suggested to be important for the process. A potential
mechanism of action of Tsix could be that transient pairing of X-chromosome
...
^59^, which in turn leads to the asymmetry in Xist expression.
...
as the barr body ^65^. The mechanism of Xist spreading has been
...
conformation to spread to regions in spatial vicinity ^66,67^. It's due to
...
the single X-chromosome in males ^70^. Males absent on first (MOF), an
...
roX1 and roX2 are lncRNAs containing stem-loop structures which 
...
other proteins in the complex, and plays an essential role in the assembly
...
important for its incorporation into the complex ^79^.
...
referred to as high-affinity sites (HAS), followed by further spreading to
...
in absence of any RNAs, and targets both, X and autosomal sites. Specificity towards the X-chromosome, 
...
shown that the spreading of the MSL complex on the X-chromosome is guided
...
that despite a global inactivation of the X-chromosome, about 15% of genes
...
mediated H4K16ac has also been implicated in embryogenesis and
oncogenesis ^90,94^. However, it should be noted that MOF is also
...
expression and the depletion of MSLs in ESCs leads to chaotic XCI during 
their differentiation. However, the lack of specificity towards the X-chromosome
and co-localization with NSL proteins at the promoters raise further
questions about the specific genome-wide role of the MSL complex, as well as
its role on the mammalian X-chromosome.
...
double-stranded RNA ^98--100^. However, in absence of its association
with the MSL complex, the function of its RNA helicase activity in mammals
...
Due to consistently reducing costs of sequencing, increase in our ability
...
ability to store, process, analyze, interpret and visualize this
data would become a major challenge for biology ^102^. Below I
will focus on recent developments in techniques employed for the study of
DNA-protein interactions, chromosome conformation and transcription.
...
genome ^17^. A ChIP-seq experiment involves DNA-protein cross-linking
(using UV or formaldehyde), chromatin fragmentation (using sonication or
enzymes), purification of DNA-protein crosslinks with or without
...
efficiency** of proteins might be influenced by their nature of DNA binding
...
enrichment and 3) **antibody cross-reactivity** could result in
detection of non-specific interactions ^104,105^. Variations of
ChIP-seq protocols as have been developed to tackle these biases, such as
...
methods might receive more popularity in the future.
...
peaks are then used to find DNA binding motifs for the TFs, detect target
genes, and integrate these results with other assays (such
...
### Analyses of chromosome conformation
...
DNA fragments. These ligated fragments could then be amplified using qPCR
primers specific to the region of interest, to measure
...
interactions ^122^. The 5C technique introduced later used highly
...
interpretation of 3C-based proximity as real molecular proximity has been
questioned ^124--126^. Recent innovations in chromosome conformation
techniques, such as GAM ^127^, SPRITE ^128^ and damC ^129^ 
...
in situ HiC as the primary method for the analysis of 3D genome.
...
genome, and is therefore symmetric. After cutting the matrix from the 
...
these applications (Appendix B2, Table-2). Detection of loops and compartments have
...
method has now been implemented in multiple tools (Appendix B2, Table-2). 
The method used for genome-wide detection of loops utilizes high-resolution in situ
...
model (HMM), which predicted the TADs. Later, methods
...
### Transcript profiling methods and analyses
...
capacity of sequencing library to capture transcripts of various lengths,
and enough input material from the cells to avoid RNA composition effect
(masking of lowly expressed transcripts by the abundant RNA). Methods have
...
vary significantly between the groups (called **differential expression** 
analysis). Differentially expressed genes are used to look
...
(\<50bp) and single-ended for most protocols, are then mapped to a 
...
this assay to promoter profiling.
